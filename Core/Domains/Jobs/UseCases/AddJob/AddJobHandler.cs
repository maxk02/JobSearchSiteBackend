    ...
    MainDataContext context) : IRequestHandler<AddJobRequest, Result>
{
    public async Task<Result> Handle(AddJobRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var jobSearchModel = new JobSearchModel(job.Id, job.JobFolderId, countryId, companyId, job.CategoryId, job.Title,
            job.Description, job.Responsibilities ?? [], job.Requirements ?? [], job.Advantages ?? []);
        
        await transaction.CommitAsync(cancellationToken);

        backgroundJobService.Enqueue(
            () => jobSearchRepository.AddAsync(jobSearchModel, CancellationToken.None),
